...
const { getProject, updateProject } = projectStore
  ...
  <div>
    <Table v-if="project">
      <TableRow>
        ...
        <TableCell>
          <EditableInput v-model="project.name" @commit="updateProject" />
        </TableCell>
      </TableRow>
      <TableRow>
        <TableHead> Description </TableHead>
        <TableCell>
          <EditableTextArea v-model="project.description" @commit="updateProject" />
        </TableCell>
      </TableRow>
      <TableRow>
        <TableHead> Status </TableHead>
        <TableCell>
          <EditableStatus v-model="project.status" @commit="updateProject" />
        </TableCell>
      </TableRow>
      <TableRow>
          ...
          <Table>
            ...
            <TableBody>
              <TableRow v-for="task in project.tasks" :key="task.id">
                <TableCell class="p-0">
                  <RouterLink
                    class="text-left block hover:bg-muted px-3 py-2"
                    :to="{ name: '/tasks/[id]', params: { id: task.id } }"
                  >
                    {{ task.name }}
                  </RouterLink>
                </TableCell>
                <TableCell> <EditableStatus readonly :model-value="task.status" /> </TableCell>
